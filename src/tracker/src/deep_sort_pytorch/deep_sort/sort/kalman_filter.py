...
class KalmanFilter(object):
    ...
    def __init__(self):
        ...
        self._motion_mat = np.eye(2 * ndim, 2 * ndim) # 生成一个8x8的单位矩阵
        for i in range(ndim):
    ...
    def initiate(self, measurement):
        ...
        """
        # 输入measurement为一个检测目标的box信息(center x, center y, w/h,height) 
        mean_pos = measurement
        # 各个速度值初始化为0。卡尔曼滤波器采用匀速模型和线性观测器模型，其观测变量为(cx,cy,r,h）
        mean_vel = np.zeros_like(mean_pos)
        mean = np.r_[mean_pos, mean_vel] # 均值(mean),表示目标的位置信息，由bbox的中心坐标（cx,cy)，宽高比r,高h,以及各自的速度变化值组成，由8维向量表示为
        ...
        # np.square(std)对std中的每个元素平方，np.diag构成一个8×8的对角矩阵，对角线上的元素是np.square(std)

...
class Tracker:
    ...
    def predict(self):
        ...
        """
        #对每个轨迹，由一个KalmanFilter预测状态分布，每个轨迹记录自己的均值和方差作为滤波器输入。
    ...
    def update(self, detections, classes):
        #执行测量更新和跟踪管理，入参detections本质是一个类，类中的各个属性和方法由一对对的box与对应的128特征向量刻画
    ...
    def _initiate_track(self, detection, class_id):
        # 由检测目标detection的坐标（(center x, center y, aspect ratio, height)构建均值向量
        #（1×8的向量，初始化为[cenx,ceny,w/h,h,0,0,0,0]与协方差矩阵(8*8矩阵，根据目标的高度构造的一个对角矩阵），
